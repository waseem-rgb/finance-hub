...
export default function RatioCard(props: RatioCardProps) {
  const { title, value, subtitle, badge, delta, valueTitle, onEvidence, onGraph, theme = "dark" } = props;
  const cardClass =
    theme === "dark"
  ...
  const valueClass = theme === "dark" ? "text-2xl font-semibold tracking-tight text-white" : "text-2xl font-semibold tracking-tight text-slate-900";
  const subtitleClass = theme === "dark" ? "text-sm text-white/70" : "text-sm text-slate-500";
    ...
    >
      <div className="grid gap-3">
        ...
        <div className="flex items-end justify-between gap-3">
          <div className="min-w-0">
            <div className={valueClass} title={valueTitle}>
              {value}
            </div>
